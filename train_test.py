...
def predict_features_dp(save_dir, version, in_dataset, palm_idx, prev_train, pool_stage, lr, MC, MC_dropout, setname):
    ...
    with tf.device("/gpu:"+GPUv):
        json_file = open("%s/hier/%s.json"%(save_dir,version_model), 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        loaded_model = model_from_json(loaded_model_json)
        if MC:
            loaded_model.load_weights("%s/hier/weight_mc_%s"%(save_dir,version))
        else:
...
def predict_dp(save_dir, version, dataset, palm_idx, lr, MC_dropout, setname):
    ...
    np.save("%s/hier/results/%s_normuvd_%s"%(save_dir,dataset,version),normuvd)
    normuvd = np.load("%s/hier/results/%s_normuvd_%s.npy"%(save_dir, dataset, version)) # change to normuvd without %s_                                                                                 
...
def predict_with_aleatoric_dp(save_dir, version, dataset, palm_idx, lr, MC_dropout, setname):
    ...
    np.save("%s/hier/results/%s_normuvd_%s"%(save_dir,dataset,version),normuvd)
    np.save("%s/hier/results/%s_aleatoric_var_%s"%(save_dir,dataset,version),aleatoric_variances)
    np.save("%s/hier/results/%s_epistemic_var_%s"%(save_dir,dataset,version),epistemic_variances)
    normuvd = np.load("%s/hier/results/%s_normuvd_%s.npy"%(save_dir, dataset, version)) # change to normuvd without %s_                                                                                 
    path='%s/source/%s.h5'%(save_dir,dataset)
    _, _, xyz_err = get_err.get_err_from_normuvd(path=path,dataset=dataset,normuvd=normuvd[:,:,:],jnt_idx=palm_idx,setname=setname)
...
def train_all_dp(save_dir, version, in_dataset, test_set, 
    ...
    while True: #
    #while not os.path.exists('%s/hier/weight_mc_%s'%(save_dir,version+'_'+str(pool_stage)+'_'+str(k))):
        print(version)
        dims = 63
        ...
        model_filepath='%s/hier/weight_mc_%s'%(save_dir,version+'_'+str(pool_stage)+'_'+str(k))
...
def train_dp_orig(save_dir, version, in_dataset, test_set, 
    ...
    # Check if the set of weights already exists, otherwise train the deep prior network
    while True: 
    #while not os.path.exists('%s/hier/weight_%s'%(save_dir,version+'_'+str(pool_stage)+'_'+str(k))):
        print(version)
        dims = 63
        keras.backend.clear_session()
        with tf.device('/gpu:'+ GPUv):
        ...
        model_filepath='%s/hier/weight_%s'%(save_dir,version+'_'+str(pool_stage)+'_'+str(k))
